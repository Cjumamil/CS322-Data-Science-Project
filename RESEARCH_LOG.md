...
## R-007
**Date:** 2026-05-05  
**Topic:** NVDA sideways-regime filter research for `vwap_rsi_mean_reversion`  
**Question:** Is the large performance gap between the 3-year and 1-year `NVDA` VWAP/RSI backtests mostly a market-regime problem, and does restricting entries to more sideways intraday conditions improve results?  
**What was tested:** First compared the saved 3-year `NVDA` run (`2023-05-05` to `2026-05-05`) against the saved 1-year run (`2025-04-21` to `2026-04-21`) and confirmed that the final year inside the 3-year test was an exact trade-for-trade match to the standalone 1-year run. Then added a new optional sideways-regime gate based on session directional efficiency and swept `max_session_directional_efficiency` values `0.25`, `0.35`, `0.45`, and `0.55` on `NVDA`. The sweeps were run both on the research baseline with `min_distance_from_vwap_frac=0.002` and on the live-like baseline with `min_distance_from_vwap_frac=0.003`. Finally, the same live-like filtered configuration was cross-checked on `AMD`, `MSFT`, and `QQQ` over both 1-year and 3-year windows.  
**Finding:** The performance gap was real regime behavior, not a rerun anomaly: the 1-year `NVDA` run exactly matched the final-year subset of the 3-year run, while the older `2023-2025` segment was materially weaker, especially on the long side. Adding the sideways-regime filter improved `NVDA` on both windows. On the live-like `0.003` baseline, 3-year `NVDA` improved from `+0.05%`, `1.02` PF, and `466` trades to `+0.21%`, `1.14` PF, and `356` trades at `max_session_directional_efficiency=0.55`. The 1-year `NVDA` run improved from `+0.28%`, `1.59` PF, and `127` trades to `+0.29%`, `1.83` PF, and `100` trades at the same threshold. However, the filter did not generalize cleanly across symbols. `AMD` became slightly worse on both windows, `MSFT` became clearly worse, and `QQQ` changed only marginally while remaining weak overall.  
**Conclusion:** The new evidence supports the idea that `NVDA` mean reversion benefits from standing down during more directionally efficient sessions, but it does not support promoting the same sideways filter across the whole `vwap_rsi_mean_reversion` symbol set. This looks like an `NVDA`-specific improvement rather than a universal VWAP/RSI upgrade.  
**Follow-up:** Keep the sideways-regime filter as an `NVDA`-only live paper-trading experiment for now. For the other symbols, continue research separately rather than forcing one shared regime rule. The next likely paths are symbol-specific threshold work or different filters altogether, such as stronger reversal confirmation or a different trend-day blocker.  
