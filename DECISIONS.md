...
## D-045
**Date:** 2026-05-04  
**Topic:** Paper-Trade Report Pipeline  
**Decision:** Add a backtest-style `paper_trade_reports/` pipeline for executed live paper trades, with account-first filtering plus optional strategy/symbol/date filters. The report workflow should reconstruct completed paper trades from local logs, save timestamped run folders, and produce artifacts parallel to the historical backtest workflow: `trades.csv`, `summary.json`, an overview equity chart, and per-trade charts. Use a visually distinct chart theme from historical backtests so paper-trade review can be recognized at a glance.  
**Reasoning:** The project already had a strong historical backtest workflow, but it lacked a parallel analysis layer for real executed paper trades. That made it harder to study how strategies were actually behaving in live paper trading, especially on a trade-by-trade basis. A dedicated paper-trade reporting pipeline preserves the familiar backtest review structure while switching the source of truth from simulated fills to actual paper-trading executions. Keeping the workflow account-aware supports clean comparison between `macd_pullback_paper` and `vwap_rsi_paper`, and saving timestamped report folders makes repeated review runs easier to organize and compare over time.  
**Approved by:** Joshua  

## D-046
**Date:** 2026-05-04  
**Topic:** Paper-Trade Report Accounting Corrections  
**Decision:** Keep the existing `paper_trade_reports/` pipeline, but correct its accounting model so paper-trade reports use account equity semantics rather than buying power for the baseline, reconstruct exits only against valid prior entries, and reconcile broker-side stop fills in an entry-aware way. Persist `equity`, `portfolio_value`, and `last_equity` into strategy-context logs for future report runs, and normalize broker-history timestamps consistently so locally persisted stop rows sort correctly.  
**Reasoning:** The first VWAP paper-trade report was financially unreliable because it treated buying power as starting equity and because some backfilled broker stop fills were being paired against later entries, producing impossible trades where `exit_time < entry_time`. Those errors inflated reported PnL far beyond the real paper-account result. The corrected approach preserves the useful existing report structure while making the reconstruction financially consistent with actual broker fills. It also reduces future dependence on live account lookups by storing true account-value fields locally in the context log, while still allowing optional Alpaca broker-history supplementation when the local trade log may be incomplete.  
**Approved by:** Joshua  

## D-047
**Date:** 2026-05-05  
**Topic:** NVDA-only live paper-config promotion for the VWAP/RSI sideways-regime filter  
**Decision:** Update the `vwap_rsi_paper` account so `NVDA` alone now uses the VWAP/RSI live-comparison settings `min_distance_from_vwap_frac=0.003`, `max_bars_in_trade=15`, `enable_sideways_regime_filter=true`, and `max_session_directional_efficiency=0.55`. Leave the rest of the `vwap_rsi_mean_reversion` symbols on the earlier shared live-comparison settings while broader symbol research continues.  
**Reasoning:** The latest NVDA backtests showed that the 1-year VWAP/RSI run was an exact trade-for-trade subset of the stronger final year inside the 3-year run, which confirmed the earlier performance gap was driven by regime differences rather than a backtest inconsistency. A new sideways-regime gate based on session directional efficiency improved NVDA on both the 3-year and 1-year windows, including the live-like `min_distance_from_vwap_frac=0.003` baseline. However, the same filter did not generalize cleanly when cross-checked on `AMD`, `MSFT`, and `QQQ`, where results were flat or worse. Because the evidence supported an NVDA-specific improvement rather than a symbol-agnostic upgrade, promoting the new regime filter only for NVDA preserves the live paper-comparison value of the account without prematurely rolling a weak cross-symbol change into the rest of the VWAP/RSI basket.  
**Approved by:** Joshua  
